...
void Battery_Monitor(void (*overheat_cb)(void)) { // tham so truyen vao la mot con tro ham
    int battery_temp = 45;
...
int main() {
    ...
    // HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
    if (my_bike.fields.ASSIST_LEVEL == Eco)
        drive_modes[0]();
    else if (my_bike.fields.ASSIST_LEVEL == Normal)
        drive_modes[1]();
    else // = Sport
        drive_modes[2]();
